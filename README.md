...
本仓库是面向开源发布整理后的代码包，代码基于 DI-engine 接入多智能体算法，核心环境为六自由度 AUV 集群目标追踪环境，提出方法为 `STG-MAPPO`。
...
```text
We implement a semantic task graph enhanced MAPPO framework for distributed AUV swarm target tracking in a six-degree-of-freedom underwater simulation environment. The implementation is built on DI-engine and supports multiple MARL baselines under identical medium/hard evaluation protocols.
```
