...
"""
Generate paper-ready figures and tables for the medium 4-AUV experiments.

This script is intentionally self-contained:
- It reads completed runs from artifacts/auv6dof_tmc_2e6/medium_4auv/auv6dof.
...
- paper_tables/medium_paper_tables.xlsx if pandas/openpyxl are available.
...
DEFAULT_BASE = REPO_ROOT / "artifacts/auv6dof_tmc_2e6/medium_4auv/auv6dof"
...
def build_tables(runs: List[RunData], table_dir: Path) -> Dict[str, Path]:
    ...
    try:
        ...
        xlsx_path = table_dir / "medium_paper_tables.xlsx"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate medium 4-AUV paper figures and tables.")
    ...
    fig_dir = out_root / "paper_figures"
    table_dir = out_root / "paper_tables"
